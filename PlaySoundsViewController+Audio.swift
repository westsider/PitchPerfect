...
//  PitchPerfect
//  imported from udacity IOS Dev course
//  Copyright © 2016 Udacity. All rights reserved.
//
import UIKit
import AVFoundation

// a convient way to have all of the alert messages in a single place
extension PlaySoundsViewController: AVAudioPlayerDelegate {
    ...
    // raw values correspond to sender tags. Used by UIButton
    enum PlayingState { case Playing, NotPlaying }

    
    // MARK: Audio Functions
    
    // try to load the audio file passed in from the RecordSoundsViewController
    func setupAudio() {
    ...
    func playSound(rate rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        ...
        // if the pitch var is not nil then execute this pitch statement
        if let pitch = pitch {
    ...
    // if you pass in true they're all enabled
